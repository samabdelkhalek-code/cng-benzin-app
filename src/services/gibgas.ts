...
export const FUEL_META: Record<FuelType, { label: string; stationLabel: string; unit: string }> = {
  cng: { label: 'CNG', stationLabel: 'CNG Tankstelle', unit: 'kg' },
  benzin: { label: 'Benzin', stationLabel: 'Benzin Tankstelle', unit: 'L' },
};
...
function mergeAndVerify(
  ...
  }).filter((s) => s.verified || s.price !== null);
}
...
const cacheKey = (lat: number, lng: number, r: number, fuel: FuelType) => `${fuel}_v1_${r1(lat)}_${r1(lng)}_${r}`;
...
async function fetchStations(lat: number, lng: number, radius: number, fuel: FuelType): Promise<Station[]> {
  ...
  // Add 1km buffer to radius when fetching to account for rounding shift
  const fetchRadius = radius + 1;
  ...
  try {
    const [osmStations, priceSources] = await Promise.all([
      ...
      Promise.race([
        axios.get<CTStation[]>(
          `${PROXY_BASE}/prices?lat=${lat.toFixed(5)}&lon=${lng.toFixed(5)}&r=${fetchRadius}&fuel=${fuel}`,
          { timeout: 15_000 }
        )
          .then(res => res.data),
...
export function useStations(lat: number | null, lng: number | null, radius: number, fuel: FuelType) {
  ...
  return useQuery({
    ...
    queryFn: () => fetchStations(rlat!, rlng!, radius, fuel),
